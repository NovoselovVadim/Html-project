...
"""
    def title(self,text,output = None):
        if output == 1:
            return str("<title>{}</title>".format(text))
        else:
...
""" + self.base[index:]
    def heading(self,text,number,output = None):
        if output == 1:
            return str("<h{}>{}</h{}>".format(str(number),text,str(number)))
        else:
            index = self.base.find("</body>")
...
"""+self.base[index:]
    def video(self,link,width="",height="",output = None):
        if output == 1:
            ...
            <source src="{}">
        </video>""".format(width, height, link))
        else:
            index = self.base.find("</body>")
...
""" + self.base[index:]
        ...
        self.base = self.base[:index] + "    </ul>" + """
""" + self.base[index:]
    def strong(self,text,output = None):
        if output == 1:
            return str("<strong>{}</strong>".format(text))
        else:
            index=self.base.find("</body>")
...
page.paragraph(page.emphasized(page.strong("This is emphasized and strong text",1),1))
page.paragraph("This is {} and {} text".format(page.small("small",1), page.small(page.strong("small-strong",1),1)))
